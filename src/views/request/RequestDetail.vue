    ...
    <div class="form-body">
      <DxScrollView :scroll-by-thumb="true">
        ...
        <div class="form-main p-24">
          <DxForm id="form" :col-count="15" :form-data="selectedRequest" :show-colon-after-label="false"
            :disabled="viewType === 1">
            <DxGroupItem :col-span="7">
              <DxSimpleItem data-field="EmployeeId">
                <DxLabel text="Người nộp đơn" css-class="input--label" />
                <DxSelectBox :data-source="employees" :value="selectedEmployeeId" display-expr="FullName"
                  value-expr="EmployeeId" search-enabled="true" search-mode="contains" search-expr="FullName"
                  :search-timeout="200" item-template="item" @value-changed="onValueEmployeeChanged">
                  <template #item="{data}">
              ...
              <DxSimpleItem data-field="DepartmentId">
                ...
                <DxSelectBox :data-source="departments" :value="selectedDepartment" :read-only="true"
                  display-expr="DepartmentName" value-expr="DepartmentId" />
              </DxSimpleItem>
              <DxSimpleItem data-field="ApplyDate" editor-type="dxDateBox" :editor-options="dateTimeOptions">
                <DxLabel text="Ngày nộp đơn" css-class="input--label" />
              </DxSimpleItem>
              <DxSimpleItem data-field="FromDate" editor-type="dxDateBox" :editor-options="dateTimeOptions">
                <DxLabel text="Làm thêm từ" css-class="input--label" />
              </DxSimpleItem>
              <DxSimpleItem data-field="BreakTimeFrom" editor-type="dxDateBox" :editor-options="dateTimeOptions">
                <DxLabel text="Nghỉ giữa ca từ" css-class="input--label" />
              </DxSimpleItem>
              <DxSimpleItem data-field="BreakTimeTo" editor-type="dxDateBox" :editor-options="dateTimeOptions">
                <DxLabel text="Nghỉ giữa ca đến" css-class="input--label" />
              </DxSimpleItem>
              <DxSimpleItem data-field="ToDate" editor-type="dxDateBox" :editor-options="dateTimeOptions">
              ...
              <DxSimpleItem data-field="OverTimeInWorkingShiftName">
                ...
                <DxSelectBox :data-source="workTimes" v-model="selectedOverTimeInWorkingShiftName"
                  @value-changed="onValueChanged" display-expr="label" value-expr="label" />
              </DxSimpleItem>
            </DxGroupItem>
            <DxGroupItem :col-span="1" />
            <DxGroupItem :col-span="7">
              <DxSimpleItem data-field="Reason" editor-type="dxTextArea" :editor-options="{height: 90}">
              ...
              <DxSimpleItem data-field="Status">
                ...
                <DxSelectBox :data-source="statuses" v-model="selectedStatus" @value-changed="onValueChanged"
                  display-expr="txt" value-expr="value" v-if="viewType === 2 || viewType === 3" />
...
import { DEFAULT_REQUEST, DATETIME_OPTIONS, DEFAULT_EMPLOYEE_LIST, DEFAULT_DEPARTMENT_LIST, WORK_TIME, REQUEST_STATUS_ARRAY, DETAIL_VIEW_TYPE } from "../../enum.js";
import { DxScrollView } from 'devextreme-vue/scroll-view'
import { locale } from "devextreme/localization";
import {
  DxForm, DxSimpleItem, DxGroupItem, DxItem, DxLabel
...
export default {
  components: {
    DxScrollView, DxForm, DxSimpleItem, DxGroupItem, DxItem, DxLabel,
    DxSelectBox, StatusCell,
    NameCell
  },
  created() {
    locale('vi');
  ...
  methods: {
    ...
    onValueEmployeeChanged(event) {
      const index = DEFAULT_EMPLOYEE_LIST.findIndex(ele => ele.EmployeeId === event.value);
      if (index > -1) {
    ...
    async getSelectedData() {
      ...
      this.selectedOverTimeInWorkingShiftName = this.selectedRequest.OverTimeInWorkingShiftName;
    ...
    editRequest() {
      ...
      this.closeDialog();
      this.$emit('toggle-dialog', {type: DETAIL_VIEW_TYPE.EDIT, selectedRequestId: this.selectedRequest.OverTimeId})
